...
import static cn.oi.klittle.era.view.KVerticalViewPager.SCROLL_STATE_SETTLING;
import static cn.oi.klittle.era.view.datepicker.WheelPicker.SCROLL_STATE_DRAGGING;
import static cn.oi.klittle.era.view.datepicker.WheelPicker.SCROLL_STATE_IDLE;
...
//        }
    ...
    /**
     * Create and return a new {@link TabLayout.Tab}. You need to manually add this using
     * {@link #addTab(TabLayout.Tab)} or a related method.
     *
     * @return A new Tab
     * @see #addTab(TabLayout.Tab)
     */
    @NonNull
    public TabLayout.Tab newTab() {
        TabLayout.Tab tab = sTabPool.acquire();
        if (tab == null) {
    ...
    public void removeTabAt(int position) {
        ...
        final TabLayout.Tab removedTab = mTabs.remove(position);
        if (removedTab != null) {
            removedTab.reset();
    ...
    public void removeAllTabs() {
        ...
        for (final Iterator<TabLayout.Tab> i = mTabs.iterator(); i.hasNext(); ) {
            final TabLayout.Tab tab = i.next();
            i.remove();
            tab.reset();
    ...
    private TabLayout.TabView createTabView(@NonNull final TabLayout.Tab tab) {
    ...
    void selectTab(final TabLayout.Tab tab, boolean updateIndicator) {
    ...
    private void dispatchTabSelected(@NonNull final TabLayout.Tab tab) {
        for (int i = mSelectedListeners.size() - 1; i >= 0; i--) {
            mSelectedListeners.get(i).onTabSelected(tab);
        }
    }

    private void dispatchTabUnselected(@NonNull final TabLayout.Tab tab) {
        for (int i = mSelectedListeners.size() - 1; i >= 0; i--) {
            mSelectedListeners.get(i).onTabUnselected(tab);
        }
    }

    private void dispatchTabReselected(@NonNull final TabLayout.Tab tab) {
        for (int i = mSelectedListeners.size() - 1; i >= 0; i--) {
    ...
    class TabView extends LinearLayout {
        ...
        void setTab(@Nullable final TabLayout.Tab tab) {
            if (tab != mTab) {
                mTab = tab;
                update();
            }
...
//                                AnimationUtils.lerp(startRight, targetRight, fraction));
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        ...
        public void onPageScrolled(final int position, final float positionOffset,
            ...
            if (tabLayout != null) {
                ...
                tabLayout.setScrollPosition(position, positionOffset, updateText, updateIndicator);//fixme 滑动条移动(系统源码)
            }
        }
        ...
        public void onPageSelected(final int position) {
            ...
            if (tabLayout != null && isOnClickTab) {
                isOnClickTab = false;//防止重复调用切换，滑动时页签闪烁。
                tabLayout.setScrollPosition(position, 0f, true, true);//fixme 滑动条移动(自己加上去的)
            }
        }
    ...
    protected static boolean isOnClickTab = false;//是否触发了Tab点击事件
