...
public class KViewDragHelper {
    private static final String TAG = "KViewDragHelper";
    ...
    private final KViewDragHelper.Callback mCallback;
    ...
    /**
     * A Callback is used as a communication channel with the KViewDragHelper back to the
     * parent view using it. <code>on*</code>methods are invoked on siginficant events and several
     * accessor methods are expected to provide the KViewDragHelper with more information
    ...
    public abstract static class Callback {
         ...
         * one of these methods, the KViewDragHelper will enter {@link #STATE_SETTLING}
         * and the view capture will not fully end until it comes to a complete stop.
         * If neither of these methods is invoked before <code>onViewReleased</code> returns,
         * the view will stop in place and the KViewDragHelper will return to
         * {@link #STATE_IDLE}.</p>
         *
         ...
         *
         * <p>KViewDragHelper may call this method multiple times for the same view even if
    ...
    /**
     * Factory method to create a new KViewDragHelper.
     *
     * @param forParent Parent view to monitor
     * @param cb        Callback to provide information and receive events
     * @return a new KViewDragHelper instance
     */
    public static KViewDragHelper create(@NonNull ViewGroup forParent, @NonNull KViewDragHelper.Callback cb) {
        return new KViewDragHelper(forParent.getContext(), forParent, cb);
    }
     ...
     * @return a new KViewDragHelper instance
     */
    public static KViewDragHelper create(@NonNull ViewGroup forParent, float sensitivity,
                                         @NonNull KViewDragHelper.Callback cb) {
        final KViewDragHelper helper = create(forParent, cb);
        helper.mTouchSlop = (int) (helper.mTouchSlop * (1 / sensitivity));
        return helper;
    }

    /**
     * Apps should use KViewDragHelper.create() to get a new instance.
     ...
     * @param forParent Parent view to monitor
     */
    private KViewDragHelper(@NonNull Context context, @NonNull ViewGroup forParent,
                            @NonNull KViewDragHelper.Callback cb) {
        if (forParent == null) {
     ...
     * The callback's {@link KViewDragHelper.Callback#onEdgeTouched(int, int)} and
     * {@link KViewDragHelper.Callback#onEdgeDragStarted(int, int)} methods will only be invoked
     ...
     * but {@link KViewDragHelper.Callback#tryCaptureView(android.view.View, int)} will not be asked permission to
     * capture this view.
     *
    ...
    public void captureChildView(@NonNull View childView, int activePointerId) {
        if (childView.getParent() != mParentView) {
            throw new IllegalArgumentException("captureChildView: parameter must be a descendant "
                    + "of the KViewDragHelper's tracked parent view (" + mParentView + ")");
        }
     ...
     * of the KViewDragHelper's knowledge).
     *
    ...
    public boolean shouldInterceptTouchEvent(@NonNull MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN: {
                ...
                // A KViewDragHelper can only manipulate one view at a time.
                if (mDragState == STATE_IDLE) {
                    final int edgesTouched = mInitialEdgesTouched[pointerId];
                    if ((edgesTouched & mTrackingEdges) != 0) {
                        mCallback.onEdgeTouched(edgesTouched & mTrackingEdges, pointerId);
     ...
     * The child order is determined using {@link KViewDragHelper.Callback#getOrderedChildIndex(int)}.
     *
     * @param x X position to test in the parent's coordinate system
     * @param y Y position to test in the parent's coordinate system
    ...
    private boolean isValidPointerForActionMove(int pointerId) {
        if (!isPointerDown(pointerId)) {
            Log.e(TAG, "Ignoring pointerId=" + pointerId + " because ACTION_DOWN was not received "
                    ...
                    + " KViewDragHelper did not receive all the events in the event stream.");
            return false;
        }
        return true;
    }
