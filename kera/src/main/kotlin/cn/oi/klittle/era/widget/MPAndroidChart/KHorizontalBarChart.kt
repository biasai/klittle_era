...
//                khorizontalBarChart {
//                    //以下偏移量；对整个图表的位置都会有影响。
//                    extraRightOffset = kpx.pixelToDp(130f)//右边偏移量
//                    //设置图例
//                    legend {}
//                    //描述
//                    description {
//                        text = "水平条型统计图"
//                        //setPosition(kpx.screenWidth().toFloat() - kpx.x(36), kpx.maxScreenHeight()-kpx.statusHeight - description.textSize*2)
//                        setPosition(kpx.screenWidth().toFloat() - kpx.x(16), kpx.x(110f))
//                    }
//                    //添加数据(支持多条)
//                    addData(entries {
//                        add(BarEntry(0f, 2f))
...
//                    //缩放和滑动到指定位置
//                    //zoom(4, 2)//条型图就不要缩放滚动了，效果不好。(测试发现，效果还行)
//                    //自定义点击选中效果
...
//                                x = (posX + airWidth).toInt() + kpx.x(10)
//                                y = posY.toInt() - rectHeight / 2
//                                all_radius = kpx.x(10f)//圆角（所有的圆角；包括气泡）
//                                isAirRadius = true//气泡是否具备圆角
...
/**
 * 水平条型图
 * fixme 正常方向；x轴：从左往右；y轴：从下往上。
 * fixme 水平条形图；x轴和y轴位置互换。
 * fixme x轴方向;从下往上（如果有滑动，也是从上面滑出）；y轴从左往右。
 * fixme zoom的时候；scaleX 和 scaleY参数位置也要互换。
 * fixme 其余用法和BarChart一样。HorizontalBarChart 是 BarChart的子类。
 */
class KHorizontalBarChart : HorizontalBarChart {
    ...
    fun zoom(visiNum: Int, toX: Float = 0F) {
        ...
        //fixme 水平条形图；x轴和y轴位置互换。所以scaleX 和 scaleY参数位置也要互换。
        zoom(1f, scaleX, xValue, 0f, YAxis.AxisDependency.LEFT)
    }

    /**
     * 设置图例；默认矩形右上角
     */
    fun legend(block: Legend.() -> Unit): KHorizontalBarChart {
     ...
     * 描述；默认设置在右下角
     */
    fun description(block: Description.() -> Unit): KHorizontalBarChart {
     ...
     */
    open fun addData(entries: ArrayList<BarEntry>, label: String, block: (BarDataSet.() -> Unit)? = null): KHorizontalBarChart {
...
//                barDataSet.setColor(Color.RED)//条型图颜色
     ...
     *
     */
    open fun addLimitLine(limit: Float, label: String, yAxis: YAxis = axisLeft, block: LimitLine.() -> Unit): KHorizontalBarChart {
        yAxis?.apply {
     ...
     * 左边的y轴
     */
    open fun axisLeft(block: YAxis.() -> Unit): KHorizontalBarChart {
        axisLeft?.apply {
            //fixme 反转（只有y轴有；x轴没有）
     ...
     * 右边的y轴
     */
    open fun axisRight(block: YAxis.() -> Unit): KHorizontalBarChart {
        axisRight?.apply {
            //fixme 反转（只有y轴有；x轴没有）
            //isInverted = true//true y轴反转(数值从上往下);false正常（数值从下往上）[默认就是false]
            //fixme 位置；
     ...
     * x轴
     */
    open fun xAxis(block: XAxis.() -> Unit): KHorizontalBarChart {
        xAxis?.apply {
...
//                    }
     ...
     */
    open fun valueFormatter(vararg labes: String, callback: ((value: Float, position: Int, label: String, labes: MutableList<String>) -> String?)? = null): KHorizontalBarChart {
        xAxis?.apply {
            //自定义轴标签（默认是数值标签）
