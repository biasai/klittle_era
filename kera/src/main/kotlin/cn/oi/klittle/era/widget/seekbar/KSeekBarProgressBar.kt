...
//            使用案例
//            KSeekBarProgressBar(this).apply {
//                dst {
...
class KSeekBarProgressBar : KView, View.OnTouchListener {
    ...
    override fun onTouch(v: View?, event: MotionEvent?): Boolean {
        ...
        event?.let {
            ...
            //滑动改变进度状态
            var p = it.x.toFloat() / w.toFloat()//event.x是相对于控件自身的。
            src?.let {
                if (it.width > 0) {
                    var left = centerX - it.width / 2
                    p = (event.x - left) / it.width
                }
            }
            if (p > 1) {
                p = 1f
    ...
    fun dst(block: KProgressbar_dst_src_Entity.() -> Unit): KSeekBarProgressBar {
        if (dst == null) {
...
//                关闭硬件加速（默认是开启，需要手动关闭一下），阴影才会有效果
//                setLayerType(View.LAYER_TYPE_SOFTWARE,null)
//                drawThumb { canvas, paint, x, y ->
//                    paint.color=Color.WHITE
//                    paint.setShadowLayer(kpx.x(10f), 0f, 0f, Color.RED)
//                    canvas.drawCircle(x,y,kpx.x(25f),paint)//画一个带阴影的圆
//                }
        ...
        model?.let {
            ...
            if (it.bg_bp == null || it.bg_bp!!.isRecycled) {
                ...
                if (isSrc) {
                    ...
                    progressX = right
                    progressY = centerY
                    //fixme 如果进度小于高度，则宽度和高度设置对等；不然圆角样式很丑！！
                    //有圆角属性时，进度宽度最好不要小于高度。不然不好看。
                    if ((right - left) < (bottom - top) && (it.left_top > 0 || it.left_bottom > 0 || it.right_top > 0 || it.right_bottom > 0)) {
                ...
                if (isSrc) {
                    ...
                    var r = it.bg_bp!!.width * progress
                    var b = it.bg_bp!!.height
                    //fixme 对图片进行百分比截取。
                    canvas.drawBitmap(it.bg_bp!!, Rect(0, 0, r.toInt(), b), Rect(left.toInt(), top.toInt(), right.toInt(), bottom.toInt()), paint)
                } else {
