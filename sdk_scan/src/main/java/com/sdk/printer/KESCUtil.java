...
//ESCUtil是我在商米科技网站上的技术文档里找到的
//常用指令封装
public class KESCUtil {
    ...
    public static byte[] getPrintQRCode2(String data, int size) {
        ...
        byte[] bytes2 = KBytesUtil.getZXingQRCode(data, size);
        return KBytesUtil.byteMerger(bytes1, bytes2);
    }
    ...
    public static byte[] getPrintBarCode(String data, int symbology, int height, int width, int textposition) {
        ...
        try {
            ...
            byte[] barcode;
            if (symbology == 10) {
                barcode = KBytesUtil.getBytesFromDecString(data);
            } else {
    ...
    public static byte[] printBitmap(Bitmap bitmap) {
        ...
        byte[] bytes2 = KBytesUtil.getBytesFromBitMap(bitmap);
        return KBytesUtil.byteMerger(bytes1, bytes2);
    }
    ...
    public static byte[] printBitmap(byte[] bytes) {
        ...
        return KBytesUtil.byteMerger(bytes1, bytes);
    }
    ...
    *   需要设置1B 33 00将行间距设为0
    ...
    public static byte[] selectBitmap(Bitmap bitmap, int mode) {
        return KBytesUtil.byteMerger(KBytesUtil.byteMerger(new byte[]{0x1B, 0x33, 0x00}, KBytesUtil.getBytesFromBitMap(bitmap, mode)), new byte[]{0x0A, 0x1B, 0x32});
    }
